...
public class PatientService {
    ...
    public Patient upsertProfile(Long userId, Patient details){
        ...
        // Check NSS uniqueness only when it has changed (or is new)
        String newNss = details.getNumeroSecuriteSociale();
        if (newNss != null && !newNss.isBlank()) {
            repo.findByNumeroSecuriteSociale(newNss).ifPresent(existing -> {
        ...
        // With @MapsId, Hibernate derives Patient.id from user.id automatically.
        // Do NOT call p.setId() manually — it causes "null identifier" on new entities.
        p.setUser(user);
